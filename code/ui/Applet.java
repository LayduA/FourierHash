...
public class Applet extends JFrame {
    ...
    public enum DMODE {
        ...
        Blockies128, Random128, Fourier256, Fourier128
    }
    ...
    private void initUI() {
        ...
        JTextArea psiDisplay = new JTextArea("psi = ");
        JButton distButtonOnce = new JButton("Psi");
        distButtonOnce.addActionListener(l -> {
            String path = "/Users/laya/Documents/applet/applet_env/Scripts/python";
            ...
            int[] pixels1, pixels2;
            pixels1 = canvas1.drawFourierHashDet(res.createGraphics(), inputL.getText(), 0,
                    MODE, prng, Surface.CUBIC, 2.0);
            pixels2 = canvas2.drawFourierHashDet(
                    res.createGraphics(), inputR.getText(), 0,
                    MODE, prng, Surface.CUBIC, 2.0);
            try {
                ...
                System.out.println("Ah non pas une erreur oh non");
            }

            psiDisplay.setText(sb.toString());
        ...
        distButton.addActionListener(l -> {
            ...
            pB.redirectErrorStream(true);
            StringBuilder sb = new StringBuilder();
            String[][] similarities = new String[7][7];
            BufferedImage res = new BufferedImage(HASH_W, HASH_H, BufferedImage.TYPE_INT_RGB);
            int[] pixels1, pixels2;
            int bitMin = 0;
            int sim;
            for (int dist = 0; dist < similarities.length; dist++) {
                for (int corr = 0; corr < 30; corr++) {
                    sim = 0;
                    double corrD = corr / 10.0;
                    for (int iter = 0; iter < 10; iter++) {
                        sb = new StringBuilder();
                        pixels1 = canvas1.drawFourierHashDet(res.createGraphics(), Crypto.getHash(corr + "" + iter), 0,
                    ...
                    similarities[dist][corr] = dist + "," + corrD + "," + (sim / 10_000.0);
                    // COMPRESSION
                    // try {
            ...
            try {
                File csvOutputFile = new File("code/similaritiesNOTrandom" + bitMin + "" + (bitMin + 5) + ".csv");
                PrintWriter pw = new PrintWriter(csvOutputFile);
                for (int i = 0; i < similarities.length; i++) {
                    for (int k = 0; k < similarities[0].length; k++) {
                        pw.print(similarities[i][k]);
                    }
            ...
            runButton.addActionListener(e -> {
                ...
                for (int i = 3; i < 35; i++) {
                    canv.drawHash(frame.getGraphics(),
                            TransformHash.flipBit(inputL.getText(), (MODE == DMODE.Fourier256 ? 3 : 1) * (i - 3) + 1),
                            i, MODE, prng);
                }
            });
