...
The way we map bits the frequences is as follows:
$$w_i = d_i(0.5 + 0.5b_{2i})\exp(b_{2i+1} \cdot \pi \cdot i)$$
...
Once the spectrum is generated, the 2DIDFT is applied. The resulting array is then scaled by the inverse of its mean in order to have a mean of 1 [bc very few have values $> 1$], and then multiplied by 255 to get an array of gray pixels. We apply the modulo 255 operator to values greater than 255 to generate the final pixel array. \Cref{fig:unfiltered} shows the resulting fingerprints of [$h$ and $h^\prime$], that differ only at bit 67. This pair yields a Haar-PSI value of 0.883, which is much more than the threshold value [OF X]
...
\begin{center}
\Cref{fig:oneFunc}- shows the two same $h$ and $h^\prime$. The PSI is 0.496

\begin{figure}
    \centering
    \includegraphics[width=3cm]{figures/OneFunc.png}
    \includegraphics[width=3cm]{figures/OneFuncP67.png}
...
Because we have to sample a lots of frequencies in order to get every bit of information, the ones on the far side of the spectrum have little to no influence on the resulting fingerprints. Furthermore, having frequencies that are so far from the origin creates some noisy images that result in a lot of small shapes, which is harder to differentiate. To address those problems, we repeat the sampling [3] times. That way we have 3 spectra, each corresponding to one color channel. The color of a pixel at position is $C(x,y) := (f(a_{red}, x, y), f(a_{green}, x, y), f(a_{blue}, x, y))$. The PSI is 0.47

\begin{center}
\begin{figure}
    \centering
    \includegraphics[width=3cm]{figures/ThreeFuncs.png}
    \includegraphics[width=3cm]{figures/ThreeFuncsP67.png}
...
Having all fingerprints being made of the same colors is certainly not a good idea. For that reason, we introduce parity bits to create fingerprints with different colors. Since we will use colors that are not purely made of one channel, we must slightly change the way we assign a color to a pixel: we must choose $2^3$ colors to account for every possibility of the filtered values of $a_0, a_1$ and $a_2$. Suppose we have an array named $Colors$ of length 8, containing rgb encodings.
